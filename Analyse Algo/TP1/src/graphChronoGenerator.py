...
def getRunTime( execfile, genefile, tempfile, input ) :
    ...
    while time() - avant < CONST_MIN_RUNTIME :
        ...
        subprocess.check_output("{} {} {}".format(genefile, tempfile, input ), shell=True)
...
def makeRuntimes( data ) :
    ...
    while x < data['borneSup'] and temps < data['attenteMax'] :
        temps = getRunTime( data['executable'], data['generator'], data['temp_file'], int(x) ) - wastedTimePerIteration
        if temps < 1e-3 :

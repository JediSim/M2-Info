...
def distance_min_naive(L): # graphe 6
    """
        Temps d'execution : O(n²)
        tps: 10⁸ * 10⁸ = 10¹⁶ => 10¹⁶/10⁷*2.90 = 344827586s = 10.9 ans
    """
    if len(L)<2:
...
def distance_min_dpr(L): # graphe 7
    """
        Temps d'execution : O(n log n)
        tps: 10⁸ * log(10⁸) = 8 * 10⁸ => (8 * 10⁸)/(10⁷*2.90) = 27s
    """
    n = len(L)

    if n <= 3:
        return distance_min_naive(L)
    ...
    strip = [point for point in L if abs(point.x - mid_point.x) < min_distance]
    strip.sort(key=lambda point: point.y)
    
    for i in range(len(strip)):
        for j in range(i + 1, min(i + 7, len(strip))):
            distance = strip[i].distance(strip[j])
